...
class herramientas_BD:
    ...
    def instancia_admin(self):
        ...
        if not c.fetchone():
            c.execute('''INSERT INTO administradores (id, permiso, firma) VALUES (?, ?, ?)''', ('root', 0, self.C.firmado('holamundo')))
    ...
    def add_admin(self, admin_id, security_phrase):
        ...
        c = self.conn_admin.cursor()
        c.execute('''INSERT INTO administradores (id, permiso, firma) VALUES (?, ?, ?)''', (admin_id, 1, firma))
        self.conn_admin.commit()
